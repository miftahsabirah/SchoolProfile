    ...
    <script>
        $(document).ready(function() {
            $(document).on('click', '.detail-btn', function() {
                ...
                window.location.href = `{{ url('/sarpras/detailsarpras') }}/${id}`;
            });
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (Array.isArray(response)) {
                        response.forEach(function(item) {
                            var imgUrl = "http://localhost:8000/storage/post_img/" + item
                            .gambar;
                            var sarprasItem = $(`<div class="relative group detail-btn"' data-id="${item.id}"'>` +
