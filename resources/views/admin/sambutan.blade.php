...
@section('content')
    ...
    <div class="bg-gray-100 flex justify-center min-h-screen">
        <div class="w-full px-12 shadow-xl rounded-lg">
            ...
            <div class="bg-white p-6 rounded-br-lg rounded-bl-lg shadow-lg w-full h-screen">
                <form id="update_sambutan" enctype="multipart/form-data">
                    <div class="mt-8">
                        <label class="block text-sm font-medium text-gray-900 mb-2">Tambahkan Deskripsi</label>
                        <div
                            class="w-full mb-12 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                            ...
                            <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
                                ...
                                <textarea id="editor_sambutan" rows="8"
                                    class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400"
                                    placeholder="Tambahkan deskripsi disini..." required>
                                <input type="text" id="sambutan" name="sambutan"></textarea>
                            </div>
                        </div>

                        <div>
                            <button type="submit"
                                class="w-full bg-blue-700 text-white p-1.5 rounded-lg hover:bg-blue-800">Simpan
                                Perubahan</button>
                        </div>
    ...
    <script>
        $.ajax({
            ...
            success: function(response) {
                if (Array.isArray(response) && response.length > 0) {
                        $('#editor_sambutan').val(response[0].sambutan_kepsek);
                    } else {
        ...
        $('#update_sambutan').submit(function(event) {
            event.preventDefault(); 

            var formData = {
                sambutan_kepsek: $('#editor_sambutan').val()
            };

            $.ajax({
                url: "http://127.0.0.1:8000/api/updatesambutan/1", 
                type: "POST", 
                dataType: "json",
                data: formData, 
                success: function(response) {
