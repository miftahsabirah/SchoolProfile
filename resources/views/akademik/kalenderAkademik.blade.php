                ...
                success: function(response) {
                    if (Array.isArray(response)) {
                        ...
                        response.forEach(function(item) {
                            var imgUrl = "http://localhost:8000/storage/imgakademik/" + item.gambar;
                            html +=
                                '<div class="gap-8 items-center lg:grid lg:grid-cols-2 xl:gap-16">';
                            html +=
                                '<img class="mb-4 w-full lg:mb-0 lg:flex rounded-lg" src="' +
                                imgUrl + '" alt="">';
                            html += '<div class="text-gray-500 sm:text-lg dark:text-gray-400">';
                            html +=
                                '<h2 class="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">';
                            html +=
                                '<span style="display: inline-block;">Tahun Akademik</span>';
                            html += '<span style="display: inline-block;">' + item
                                .tahun_ajaran + '</span>';
                            html += '</h2>';
                            html += '<p class="mb-8 font-light lg:text-xl">' + item.deskripsi +
                                '</p>';
                            var downloadUrl = "http://localhost:8000/storage/akademik/" + item
                                .file_berkas;
